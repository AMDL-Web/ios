...
extension JobStatus {
    ...
    /// 而 iOS 的惯例是删除贴着屏幕右边缘，正好跟菜单顺序相反。
    /// 反转而不是另写一张表 —— 可用性只有一个来源。
    var trailingSwipeActions: [JobAction] {
        availableActions.reversed()
    }
}
...
enum JobActionOutcome: Equatable, Sendable {
    ...
    /// 已删除。HTTP 200 就是「这条真的没了」，可以直接摘行、退出详情页。
...
final class JobActionRunner {
    ...
    /// 待确认的破坏性动作。非空时列表/详情页弹确认框。
    var pendingConfirmation: JobActionRequest?
    var failure: JobActionFailure?

    struct JobActionRequest: Identifiable {
        let id = UUID()
        let action: JobAction
        let job: Job
    }
    ...
    /// 用户点了某个动作。需要确认的先存起来等确认，其余直接执行。
    /// 返回值只在直接执行时有意义。
    @discardableResult
    func request(_ action: JobAction, on job: Job) async -> JobActionOutcome? {
        guard !action.requiresConfirmation else {
            pendingConfirmation = JobActionRequest(action: action, job: job)
            return nil
        }
        return await run(action, on: job)
    }
...
extension View {
    /// 动作失败的 alert 与破坏性动作的确认框。两个入口挂同一套，措辞才不会分叉。
    func jobActionPrompts(
        runner: JobActionRunner,
        onConfirmed: @escaping (JobActionOutcome) -> Void
    ) -> some View {
        self
            ...
            .confirmationDialog(
                confirmationTitle(for: runner.pendingConfirmation),
                isPresented: Bindable(runner).pendingConfirmation.isPresent(),
                titleVisibility: .visible,
                presenting: runner.pendingConfirmation
            ) { request in
                // 动作和任务都从 `presenting` 给的 `request` 里取，**不要**回头去读
                // `runner.pendingConfirmation`：SwiftUI 会先把 `isPresented` 置回
                // false（经由 `isPresent()` 的 setter 把 pendingConfirmation 清空），
                // 之后才执行这个闭包。回读的话每次都读到 nil，确认框一关，什么也没发生。
                Button(request.action.title, role: .destructive) {
                    Task {
                        if let outcome = await runner.run(request.action, on: request.job) {
                            onConfirmed(outcome)
                        }
                    }
                }
                ...
                Text("删除后无法恢复。已经下载好的文件不会被删除。")
            }
    }
}

private func confirmationTitle(for request: JobActionRunner.JobActionRequest?) -> String {
    guard let request else { return "" }
    return "\(request.action.title)「\(request.job.displayName)」？"
}

extension Binding {
    /// `Binding<T?>` → `Binding<Bool>`，置 false 时清空。`confirmationDialog` 要
    /// `isPresented` 和 `presenting` 两个参数，而 `.alert(item:)` 那种单值形式
    /// 在 confirmationDialog 上没有对应版本。
