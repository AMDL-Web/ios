...
struct DownloadView: View {
    ...
    /// 显示「正在停止…」。
    ///
    /// 删除到不了这里：侧滑手势不给删除，能删的只有详情页的 ⋯ 菜单。那边删掉的行
    /// 由事件流的 `download_deleted` 落到 `apply(_:)` 里摘掉。
    private func handle(outcome: JobActionOutcome) {
        ...
        case .cancelling, .requeued, .deleted:
            break
        }
    }
        ...
        if !jobs.isEmpty {
            Section(title) {
                ForEach(jobs) { job in
                    ...
                    // 侧滑里没有「删除」（见 `trailingSwipeActions`），剩下的
                    // 「停止 / 重新开始」也一样是误触了就得重来一遍的操作，
                    // 所以仍然不让一次划到底直接触发。
                    .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                        ForEach(job.status.trailingSwipeActions) { action in
                            Button {
                                Task {
                                    if let outcome = await actionRunner.run(action, on: job) {
